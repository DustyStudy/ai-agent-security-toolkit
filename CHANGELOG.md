...
- On Windows, every file the CLI and `ToolPins.save` write (`threatmodel render`/`init`, `fuzz --json`/`--md`/`--sarif`/`--junit`, MCP tool pins) came out CRLF instead of LF, because `Path.write_text` translates `\n` to the platform line separator by default. A rendered threat model no longer matched its committed LF original line-for-line, and committing a regenerated report or pin file from Windows produced an all-lines-changed diff. These writes now pass `newline="\n"` so output is byte-identical across platforms; the toolkit already claims Windows support and CI runs the suite there, but the one job that diffs generated output against a committed file (`toolkit-self-checks`) only ran on Linux, so this went uncaught. Real before/after output and the passing Windows CI run are recorded in [`docs/windows-crlf-fix-verification.md`](docs/windows-crlf-fix-verification.md).
...
### Changed
- CI and the release workflow install Python tooling from hash-pinned, cross-platform requirement files (`requirements/`, generated by `scripts/lock.py` with a pinned `uv`) using `pip install --require-hashes`. A CI job fails if they no longer match `pyproject.toml`. The SBOM is generated from the pinned runtime dependencies, so it is reproducible.
